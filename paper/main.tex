...
\begin{proposition}[Canonical inference state, after \citet{moore1956} and \citet{nerode1958}]\label{thm:canonical}
...
\centering
\begin{tikzpicture}
\begin{axis}[width=.83\linewidth,height=2.7cm,xmin=1,xmax=16,ymin=1,ymax=16,
...
\section{What an artifact is obliged to retain}\label{sec:checks}
Learning on artifacts has a representation-independent target: the inference quotient, its decompositions, and the maps between them. The quotient is what two artifacts share when they implement the same inference; the exchange law separates architectures of equal state cardinality; the hardening certificate names the transformations that preserve all future inference. In a model population \citep{schurholt2022modelzoos} the extracted object answers which observations separate models, which queries are already determined, and which experiments change the answer. All results in this paper are verified in Lean~4, and the constructions are executed exhaustively on the finite families of \cref{tab:checks}, each enumeration defined in \cref{app:checks}.

\begin{table}[t]
\caption{Exhaustive execution of the constructions. Counts are enumerated objects or input cases.}
\label{tab:checks}
\centering
\small
