...
The second number is the height $\height(\R)$, the number of elements in a longest chain. Along any history the states $[V(s)]$ descend in $\R$, so the readout changes at most $\height(\R)-1$ times, and the Boolean store with first-consistent-model selection attains this on the history that eliminates candidates from the bottom. Cardinality is what a coupled store must hold; height is what an independent one must hold (\cref{sec:exchange}).
...
\end{proof}
The separation belongs to chains: the ratio $W_{\mathrm{ind}}/W_{\mathrm{joint}}$ is controlled by $\height(\R)/\log_b|\R|$, which is $n/\log_bn$ on $C_n$ and $1+1/k$ on $2^{[k]}$.
...
\end{equation}
Assign distinct labels within each fibre and reuse them across fibres. This construction need not support independent updates.
...
\centering
\begin{tikzpicture}
\begin{axis}[width=.83\linewidth,height=3.5cm,xmin=1,xmax=16,ymin=1,ymax=16,
...
Most results are formalized in Lean~4 with Mathlib. The constructions are additionally verified exhaustively on finite families. The accompanying programs use exact integer arithmetic for the transition and congruence checks, and NumPy float64 for softmax evaluations. The network artifacts are compiled from the specified grammars. \Cref{tab:checks} reports the executed checks; \cref{app:checks} defines every enumeration and its assertions.
